...
export const getGpViewerPickedRiders = unstable_cache(
	async (gpId: number, viewerId: number) =>
		dataFetch(async () => {
...
export const getGpUserResult = unstable_cache(
	(gpId: number, userId: number) =>
		dataFetch(
			() =>
				db
...
export const getGpTopUsers = unstable_cache(
	(gpId: number, limit = 3) =>
		dataFetch(
			() =>
				db
...
export const getGp = unstable_cache(
	(id: number) =>
		dataFetch(
			() =>
				db
...
export const getGpRidersResults = unstable_cache(
	(gpId: number, viewerId?: number) =>
		dataFetch(
			() =>
				db
					.selectFrom('riders_results')
...
export const getGpUsersResults = unstable_cache(
	(gpId: number) =>
		dataFetch(
			() =>
				db
...
export const getGpRidersPreview = unstable_cache(
	...
	['gp-riders-preview'],
	{ tags: [cacheTags.gpResults, cacheTags.picks] }
)
