			...
			{items.map((item) => {
				...
				return (
					<li key={item.href}>
						<Link
							href={item.href}
							className={`flex flex-col items-center gap-1 text-sm leading-tight transition-colors hover:text-foreground ${isActive ? 'text-brand-red' : 'text-muted-foreground'}`}
						>
